<?php
  class Validate extends DBQueries {
    use TestInput;
    ...
    public function __construct($initDetails) {
      ...
      $this->firstNameErr=$this->lastNameErr=$this->ageErr=$this->genderErr=$this->emailErr=$this->usernameErr=$this->passwordErr=$this->signupErr='';
    }
    ...
    final public function val_name(string $name, string $category) {
      if (empty($name)) {
        ...
        if (strlen($name) < 3) {
          if ($category === 'firstName') {
            $this->firstNameErr = 'Name too short!! Min 3 chars<br>';
          } elseif ($category === 'lastName') {
            $this->lastNameErr = 'Name too short!! Min 3 chars<br>';
          } else {
            die ('Invalid Category');
          }
        } elseif (strlen($name) > 100) {
          if ($category === 'firstName') {
            $this->firstNameErr = 'Name too long!! Max 100 chars<br>';
          } elseif ($category === 'lastName') {
            $this->lastNameErr = 'Name too long!! Max 100 chars<br>';
          } else {
            die ('Invalid Category');
          }
        } elseif (!filter_var($name, FILTER_VALIDATE_REGEXP, ['options' => ['regexp' => '/^[a-zA-Z\s]*$/']])) {
          if ($category === 'firstName') {
            $this->firstNameErr = 'Only leter charecters and white spaces are allowed<br>';
          } elseif ($category === 'lastName') {
            $this->lastNameErr = 'Only leter charecters and whire spaces are allowed<br>';
          } else {
            die ('Invalid Category');
          }
        } else {
    ...
    final public function val_username(string $username) {
      if (empty($username)) {
        ...
        } elseif (!filter_var($username, FILTER_VALIDATE_REGEXP, ['options' => ['regexp' => '/^[a-z0-9._]*$/']])) {
